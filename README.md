...
- Detects external drives at mount time and can clean each stable mount once, after a short delay, if you enable that preference.
...
Automatic cleanup is **off by default**. If you enable it, DriveSweep waits briefly after mount and cleans each stable eligible mount once. When you finish copying, use **Clean and eject** from the DriveSweep menu for a final pass.

Removing `._*` files may discard macOS-only metadata such as custom icons or legacy resource forks. By default DriveSweep enables AppleDouble and `.DS_Store` cleanup; all other categories require an explicit opt-in. Use the AppleDouble extension whitelist for types whose resource metadata must be preserved.

## Security and Apple verification

DriveSweep **is free**, but the `v0.2.2` public build is **not notarized by Apple**. That is why macOS can show “Apple could not verify that this app is free of malware” for a DMG downloaded from GitHub. This warning is about Apple's distribution verification process; it is not a report that DriveSweep is malware.
...
For `v0.2.2`, the published SHA-256 is:

```text
09e91de9679695581de14521f0d4b20ee192f562a636deb5902979ec205c4d4a
```
