...
Removing `._*` files may discard macOS-only metadata such as custom icons or legacy resource forks. By default DriveSweep enables only `.DS_Store` cleanup; AppleDouble and all other categories require an explicit opt-in. Use the AppleDouble extension whitelist for types whose resource metadata must be preserved.

## Security and Apple verification

DriveSweep **is free**, but the `v0.2.0` public build is **not notarized by Apple**. That is why macOS can show “Apple could not verify that this app is free of malware” for a DMG downloaded from GitHub. This warning is about Apple's distribution verification process; it is not a report that DriveSweep is malware.
...
For `v0.2.0`, the published SHA-256 is:
